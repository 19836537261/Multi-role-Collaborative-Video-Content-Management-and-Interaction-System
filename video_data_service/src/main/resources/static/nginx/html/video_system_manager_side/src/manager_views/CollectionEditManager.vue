  ...
  methods: {
    handleInput(event){
      if (event!=""){
        ...
        for (let i = 0; i < this.videoList.length; i++) {
          if (this.videoList[i].title.includes(event)){
            temp_video_list[counter++]=this.videoList[i];
          }
        }
        this.videoList=temp_video_list;
      }else {
        this.videoList=this.sourceVideoList;
      }
    },
    ...
    get_category_video(){
      this.$http({
        ...
        this.sourceVideoList=this.videoList;
